...
function traverse_(M, F) {
    const applyFirst = (mu, mb) => M.ap(M.map(mu, function_js_1.constant), mb);
...
function getFoldableComposition(F, G) {
    ...
    return {
        reduce: (fga, b, f) => (0, function_js_1.pipe)(fga, _reduce(b, f)),
        foldMap: (M) => {
            const foldMapM = _foldMap(M);
            return (fga, f) => (0, function_js_1.pipe)(fga, foldMapM(f));
        },
        reduceRight: (fga, b, f) => (0, function_js_1.pipe)(fga, _reduceRight(b, f))
    };
}
