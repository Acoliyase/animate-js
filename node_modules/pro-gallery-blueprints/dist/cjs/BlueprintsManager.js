...
const Blueprints_js_1 = tslib_1.__importDefault(require("./Blueprints.js"));
...
class BlueprintsManager {
    ...
    async createBlueprint(params = {}) {
        ...
        this.currentState.isUsingCustomInfoElements =
            params.isUsingCustomInfoElements ||
                (this.api.isUsingCustomInfoElements &&
                    this.api.isUsingCustomInfoElements()) ||
                this.currentState.isUsingCustomInfoElements;
        params = Object.assign(Object.assign({}, params), (await this.completeParams(params)));
        ...
        const _createBlueprint = async (args) => {
            ...
            if (this.api.createBlueprintImp) {
                ...
                return await Blueprints_js_1.default.createBlueprint(args);
            }
        };
    ...
    async getMoreItems(currentItemLength) {
        ...
        else if (this.existingBlueprint.options[pro_gallery_lib_1.optionsMap.behaviourParams.gallery.horizontal.loop]) {
    ...
    createInitialBlueprint(params) {
        ...
        this.currentState.isUsingCustomInfoElements =
            params.isUsingCustomInfoElements ||
                ...
                this.currentState.isUsingCustomInfoElements;
        params = this.duplicateItemsForSlideshowLoopIfNeeded(params);
        const { blueprint, changedParams, reasons } = Blueprints_js_1.default.createBlueprint({
            params,
            lastParams: this.currentState,
            existingBlueprint: this.existingBlueprint,
    ...
    createSingleBlueprint(params = {}) {
        ...
        const { blueprint } = Blueprints_js_1.default.createBlueprint({
            params,
            lastParams: {},
            existingBlueprint: {},
    ...
    duplicateItemsForSlideshowLoopIfNeeded(params) {
        ...
        const isHorizontalScrolling = options[pro_gallery_lib_1.optionsMap.layoutParams.structure.scrollDirection] === pro_gallery_lib_1.GALLERY_CONSTS[pro_gallery_lib_1.optionsMap.layoutParams.structure.scrollDirection].HORIZONTAL;
        // If slideshowLoop is True and both conditions are True as well then we duplicate number of items to reach threshold
        if (options[pro_gallery_lib_1.optionsMap.behaviourParams.gallery.horizontal.loop] && numOfItemsCondition && isHorizontalScrolling) {
            const duplicateFactor = Math.ceil(loopThreshold / items.length) - 1;
            return Object.assign(Object.assign({}, params), { items: this.duplicateGalleryItems({ items, duplicateFactor }) });
        }
        return params;
    }
    ...
    updateLastParamsIfNeeded({ items, container, options }, changedParams, blueprintCreated) {
        if (blueprintCreated) {
            this.currentState.items = changedParams.itemsChanged
                ? items
                : this.currentState.items;
            this.currentState.container = changedParams.containerChanged
                ? Object.assign({}, container) : this.currentState.container;
            this.currentState.options = changedParams.optionsChanged
                ? Object.assign({}, options) : this.currentState.options;
        }
    }
