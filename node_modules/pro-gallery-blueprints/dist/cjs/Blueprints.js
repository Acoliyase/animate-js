...
class Blueprints {
    createBlueprint({ params, lastParams, existingBlueprint, blueprintManagerId, isUsingCustomInfoElements }) {
        ...
        try {
            ...
            const { formattedOptions, changed: optionsChanged } = this.formatOptionsIfNeeded(newOptions, oldOptions, isUsingCustomInfoElements);
            const { formattedContainer, changed: containerChanged } = this.formatContainerIfNeeded(newContainerParams, oldContainerParams, oldOptions, formattedOptions || existingBlueprint.options, optionsChanged);
            ...
            if (changed || !existingBlueprint) {
                ...
                const isInfinite = pro_gallery_lib_1.utils.isHeightSetByGallery(existingBlueprint.options);
                if (isInfinite) {
    ...
    formatItemsIfNeeded(items, lastItems) {
        ...
        if (itemsWereAdded(items, oldItemsParams)) {
            formattedItems = oldItemsParams.concat(items.slice(oldItemsParams.length).map((item) => {
            ...
            formattedItems = items.map((item) => Object.assign(pro_layouts_1.ItemsHelper.convertDtoToLayoutItem(item)));
    ...
    formatOptionsIfNeeded(options, lastOptions, isUsingCustomInfoElements) {
        ...
        if (optionsHaveChanged(options, oldOptions)) {
            const optionsOverDefaults = (0, pro_gallery_lib_1.populateWithDefaultOptions)(options); //add default for any undefined option
            formattedOptions = (0, pro_gallery_lib_1.processLayouts)((0, pro_gallery_lib_1.addPresetOptions)(optionsOverDefaults), isUsingCustomInfoElements);
            changed = true;
        }
    ...
    formatContainerIfNeeded(container, lastContainer, lastOptions, formattedOptions, optionsChanged) {
        const containerHasChanged = ({ newContainerParams, oldContainerParams, oldOptions }) => {
            ...
            const containerHasChanged = {
                height: pro_gallery_lib_1.utils.isHeightSetByGallery(formattedOptions) // height doesnt matter if the new gallery is going to be vertical
                    ? false
                    : !!newContainerParams.height && newContainerParams.height !== oldContainerParams.height,
                width: !oldContainerParams || (!!newContainerParams.width && newContainerParams.width !== oldContainerParams.width),
                scrollBase: (newContainerParams === null || newContainerParams === void 0 ? void 0 : newContainerParams.scrollBase) !== (oldContainerParams === null || oldContainerParams === void 0 ? void 0 : oldContainerParams.scrollBase),
            };
