export function prepareItemsToMergeIntoPage(patches, { itemPredicate, currentItemsMap, }) {
    const result = patches.reduce((acc, optimisticPatch) => {
        const { patchedItemsEntries, actionType, keepPosition, selector } = optimisticPatch;
        ...
        if (actionType === 'deleteAll') {
            for (const [key, { item }] of Array.from(acc.all.entries())) {
                if (selector(item)) {
                    acc.all.delete(key);
                    acc.deleted.set(key, true);
                }
        ...
        for (const [key, item] of patchedItemsEntries) {
            ...
            if (keepPosition && prevPatch) {
                ...
                acc.all.set(key, { item, optimisticPatch });
            }
        }
        return acc;
    }, {
    ...
    return {
        ...
        created: result.createdPatches.reverse().reduce((prev, optimisticPatch) => {
            const { patchedItemsEntries } = optimisticPatch;
            for (const [key, item] of patchedItemsEntries) {
                if (!result.deleted.get(key)) {
                    prev.set(key, { item, optimisticPatch });
                }
            }
            return prev;
        }, new Map()),
