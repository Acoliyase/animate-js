 ...
 * @returns
 */function toURLSearchParams(params,isComplexRequest){const flatten=flattenParams(params);// payloads with deep arrays or complex arrays are not serializable as URLSearchParams
const isPayloadNonSerializableAsUrlSearchParams=Object.entries(flatten).some(_ref2=>{let[key,value]=_ref2;return key.includes('.')||Array.isArray(value)&&value.some(v=>typeof v==='object');});const shouldSerializeToRParam=isComplexRequest&&isPayloadNonSerializableAsUrlSearchParams;if(shouldSerializeToRParam){return new URLSearchParams({'.r':encode(JSON.stringify(params),true)});}else{return Object.entries(flatten).reduce((urlSearchParams,_ref3)=>{let[key,value]=_ref3;// inorder to make `foo: [1,2]` turn into `foo=1&foo=2` and not `foo[]=1&foo[]=2`
const keyParams=Array.isArray(value)?value:[value];keyParams.forEach(param=>{if(param===undefined||param===null||Array.isArray(value)&&typeof param==='object'){return;}urlSearchParams.append(key,param);});return urlSearchParams;},new URLSearchParams());}}
