...
import { PricingOptionService, PricingOptionServiceDefinition, } from '../../services/pricing-option-service.js';
...
import { getTicketDefinitionFee, getTicketDefinitionTax, isTicketDefinitionAvailable, } from '../../utils/ticket-definition.js';
...
export function Root(props) {
    ...
    const pricingOptionServiceConfig = {
        pricingOption,
    };
    return (_jsx(WixServices, { servicesMap: createServicesMap().addService(PricingOptionServiceDefinition, PricingOptionService, pricingOptionServiceConfig), children: children }));
}
/**
...
export function Fee(props) {
    ...
    const price = Number(pricingOption.price.value);
    const currency = pricingOption.price.currency;
    const { rate, value, formattedValue } = getTicketDefinitionFee(taxSettings, price, currency, false, props.locale);
    return props.children({
        rate,
        value,
        currency,
...
export function Quantity(props) {
    ...
    const quantity = ticketDefinitionListService.getCurrentQuantity(ticketDefinitionId, pricingOptionId);
