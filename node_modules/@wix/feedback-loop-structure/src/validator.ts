    ...
    .superRefine((val, ctx) => {
      ...
      if (allQuestionIds.length === 0) {
        ctx.addIssue({
          ...
          message:
            'You are required to provide at least one component that defines a question',
        });
      }
...
}) satisfies ZodType<IFeedbackFormStructure>;

export const validate = (structure: IFeedbackFormStructure) => {
  const result = schema.safeParse(structure) as SafeParseReturnType<
