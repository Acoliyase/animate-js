import { withoutIrrelevant } from '@wix/bex-core';
import { cairoDragActionUnsuccessfulUpdateInServer, cairoDragItem, cairoDragItemEndDrop, cairoTryAgainClicked, } from '@wix/bex-core/bi';
import { action } from 'mobx';
const withoutDefaults = withoutIrrelevant();
export class CollectionDragAndDropBI {
    constructor(params) {
        this.startTime = 0;
        this.onAttemptDragStart = action((itemId) => {
            if (!this.dnd) {
                return () => { };
            }
            const { reportBi, dnd, collection, getBulkSelectCount } = this;
            const { _isHandleActive } = dnd;
            const { query: { search }, } = collection;
            const item = collection.getKeyedItem(itemId);
            const startTime = performance.now();
            reportBi(withoutDefaults(cairoDragItem)({
                itemId,
            ...
            return action(() => {
                reportBi(withoutDefaults(cairoDragItemEndDrop)({
                    isFromHandle: _isHandleActive,
                    itemId,
                    itemIndexBefore: item?.index,
    ...
    onDragStart() {
        ...
        const { reportBi, dnd, collection, getBulkSelectCount } = this;
        const { active, _isHandleActive } = dnd;
        const { query: { search }, } = collection;
        if (active == null) {
            return;
        }
        ...
        return (params) => {
            ...
            reportBi(withoutDefaults(cairoDragItemEndDrop)({
                ...
                endType: params.endType ??
                    (overId === active.key ? 'drop back' : 'drop succeeded'),
                numItemsBefore: getBulkSelectCount(),
                itemIndexBefore: active.index,
                itemIndexAfter: activeAfter?.index,
                dragAndDropID: dnd._moveId,
            }));
        };
    }
    onMoveFailure(params) {
        const { reportBi, getBulkSelectCount } = this;
        const { reason, patches } = params;
        ...
        const { from, over, moveId, isFromHandle } = errorPatch.move;
        reportBi(withoutDefaults(cairoDragActionUnsuccessfulUpdateInServer)({
    ...
    onMoveCancelled(move) {
        const { reportBi, getCollectionSnapshot, getBulkSelectCount, getPendingMovePatchesCount, } = this;
        const { from, over, moveId, isFromHandle } = move;
        reportBi(withoutDefaults(cairoDragActionUnsuccessfulUpdateInServer)({
    ...
    onMoveRetry(params) {
        const { reportBi, collection } = this;
        const { patches } = params;
        const { query: { search }, } = collection;
        const errorPatch = patches[0];
        reportBi(withoutDefaults(cairoTryAgainClicked)({
            ...errorPatch.collectionSnapshot,
            isFromSearch: !search.isEmpty,
            numOfWaitingUpdates: patches.length,
            actionName: 'drag',
