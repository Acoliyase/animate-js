...
import { CustomModalLayoutUniDriver, HeadingUniDriver, ModalUniDriver, RadioGroupUniDriver, TextUniDriver, } from '@wix/design-system/dist/testkit/unidriver';
export function ExportModalUniDriver(base, body) {
    ...
    const getAbsoluteCounter = () => TextUniDriver(linearProgressBarBase().$('[data-hook="progress-count"]'), body);
    const getTotalCounter = () => HeadingUniDriver(linearProgressBarBase().$('[data-hook="progress-total"]'), body);
    const getExportRadioGroup = () => RadioGroupUniDriver(modalBase().$('[data-hook="export-group"]'), body);
    const getModal = () => ModalUniDriver(modalBase(), modalBase());
    const getModalLayout = () => CustomModalLayoutUniDriver(modalBase().$('[data-hook="export-modal-layout"]'), body);
    const getFooterText = async () => (await TextUniDriver(footerBase(), body)).getText();
    const footerExists = () => footerBase().exists();
    const getCsvMessageText = async () => (await TextUniDriver(modalBase().$('[data-hook="export-csv-message"]'), body).getText()).replace('&nbsp;', ' ');
    return {
        ...
        getModalLayout,
        getCsvMessageText,
        waitModalClosed: (timeout) => waitFor(async () => !(await getModalLayout().exists()), timeout),
...
export default ExportModalUniDriver;
