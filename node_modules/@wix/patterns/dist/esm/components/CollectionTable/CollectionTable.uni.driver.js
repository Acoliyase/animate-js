import { BulkActionToolbarUniDriver } from '../BulkActionToolbar/BulkActionToolbar.uni.driver';
...
import { LoaderUniDriver, MessageModalLayoutUniDriver, TableUniDriver, LiveRegionUniDriver, PopoverUniDriver, } from '@wix/design-system/dist/testkit/unidriver';
...
export const CollectionTableUniDriver = (base, body) => {
    const table = TableUniDriver(base, body);
    const toolbar = CollectionToolbarUniDriver(base.$('[data-hook="collection-toolbar"]'), body);
    const getLiveRegion = () => LiveRegionUniDriver(body.$('[data-hook="collection-live-region"]'), body);
    ...
    const bulkActionToolbar = BulkActionToolbarUniDriver(base.$('[data-hook="bulk-action-toolbar"]'), body);
    const multiBulkActionToolbar = MultiBulkActionToolbarUniDriver(base.$('[data-hook="bulk-action-toolbar"]'), body);
    const tagList = AppliedFiltersTagListUniDriver(base.$('[data-hook="collection-table-sub-toolbar"]'), body);
    const getExportModal = () => ExportModalUniDriver(base, body);
    const getExportButton = () => ExportButtonUniDriver(base.$('[data-hook="export-button"]'), body);
    ...
    const getReorderModeToolbar = () => ReorderModeToolbarUniDriver(base.$('[data-hook="reorder-mode-toolbar"]'), body);
    ...
    return {
        ...
        ...table,
        getToolbar: () => toolbar,
        isHeaderHiddenAccessible: async () => (await base.$('[data-hook="titlebar-a11y-wrapper"]')._prop('style'))
            .display === 'none',
        /** @private */
        ...
        waitRefreshLoaderRemoved: (timeout) => waitFor(async () => !(await refreshLoaderBase().exists()), timeout),
        ...
        getAppliedFiltersTagListDriver: () => AppliedFiltersTagListUniDriver(base.$('[data-hook="collection-table-sub-toolbar"]'), body),
         ...
         */
        getConfirmActionModal: () => MessageModalLayoutUniDriver(body.$('[data-hook="collection-action-modal"]'), body),
        getTopNotification: () => TableTopNotificationUniDriver(base.$('[data-hook="table-top-notification"]'), body),
        /** Get tabs filter driver */
        getToolbarTabs: () => TabsFilterUniDriver(base.$('[data-hook="collection-tabs-filter"]'), body),
        ...
        _getInfoIconTooltipPopoverAt: (colNum) => PopoverUniDriver(base
            .$('thead')
            .$$(`th`)
            .get(colNum)
            .$('[data-hook="infoicon-tooltip"]'), body),
    };
};
