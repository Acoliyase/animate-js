...
exports.CollectionDropdownFilterUniDriver = void 0;
...
const CollectionDropdownFilterUniDriver = (optionsDriver, dataHook, base, body) => {
  const waitForOptions = async (total = 1, timeout = 3000) => {
    await (0, _unidriver2.waitFor)(async () => (await optionsDriver.dropdownLayoutDriver.optionsLength()) >= total, timeout);
  ...
  const getAvailableOptionsIds = async () => {
    ...
    const maybeIds = await Promise.all(options.map(async option => {
      ...
      return hook == null ? void 0 : hook.replace(/^dropdown-item-/, '');
    }));
  ...
  const isErrorStateExists = async () => (await getDropdownLayoutBase()).$('[data-hook="filter-error-state"]').exists();
  const getEmptyStateText = async () => (0, _unidriver.TextUniDriver)((await getDropdownLayoutBase()).$('[data-hook="empty-state-text"]'), body).getText();
  const initialLoaderExists = async () => (await getDropdownLayoutBase()).$('[data-hook="initial-loader"]').exists();
  const infiniteScrollLoaderExists = async () => (await getDropdownLayoutBase()).$('[data-hook="dropdownLayout-loader"]').exists();
  ...
  const getRandomOptions = async count => {
    ...
    const maybeIds = await Promise.all(options.map(async option => {
      ...
      return dataHook == null ? void 0 : dataHook.replace(/^dropdown-item-/, '');
    }));
    const ids = maybeIds.filter(value => value != null);
    return (0, _shuffle.default)(ids).slice(0, count);
  };
  const selectOptions = async (ids, {
    closeDropdown = true
  } = {}) => {
    await optionsDriver.inputDriver.click();
...
exports.CollectionDropdownFilterUniDriver = CollectionDropdownFilterUniDriver;
