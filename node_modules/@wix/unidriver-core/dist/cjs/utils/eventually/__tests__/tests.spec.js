...
const eventually_1 = require("../eventually");
describe('eventually', () => {
    describe('Pass', () => {
        it('should with async function', async () => {
            // eslint-disable-next-line @typescript-eslint/no-confusing-void-expression
            const result = await (0, eventually_1.eventually)(async () => undefined);
            expect(result).toBe(undefined);
        });
        it('should with sync function', async () => {
            // eslint-disable-next-line @typescript-eslint/no-confusing-void-expression
            const result = await (0, eventually_1.eventually)(() => undefined);
            expect(result).toBe(undefined);
        });
        it('should with sync function returning string', async () => {
            // eslint-disable-next-line @typescript-eslint/no-confusing-void-expression
            const result = await (0, eventually_1.eventually)(() => 'SOME_STRING');
            expect(result).toBe(undefined);
        });
        it('should pass from 2-nd time', async () => {
            let tryCounter = 1;
            // eslint-disable-next-line @typescript-eslint/no-confusing-void-expression
            const result = await (0, eventually_1.eventually)(async () => {
                if (tryCounter === 2) {
                    return;
                }
                tryCounter++;
    ...
    describe('Not pass', () => {
        it('should retry for 1 second with 30 ms delay by default and fail', async () => {
            ...
            try {
                await (0, eventually_1.eventually)(() => {
        ...
        it('should be able to work with shorthand timeout param', async () => {
            try {
                await (0, eventually_1.eventually)(async () => {
                    throw new Error('error');
                }, 10);
            }
            catch (e) {
                expect(e).toBeInstanceOf(TimeoutError_1.TimeoutError);
        ...
        it('should show id if defined and error is thrown', async () => {
            try {
                await (0, eventually_1.eventually)(async function test() {
                    throw new Error('error');
                }, {
                    timeout: 10,
