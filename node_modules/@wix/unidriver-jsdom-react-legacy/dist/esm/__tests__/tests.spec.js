...
const renderTestApp = (element, props) => {
    const comp = createElement(TestApp, props);
...
describe('Adapter Specific', () => {
    ...
    describe('click', () => {
        it('sends event data properly on simulated events when element is not attached to body', async () => {
            const s = vi.fn();
            const { container } = render(React.createElement("button", { onClick: s }, "bob"));
        ...
        it('should trigger [mouseDown, mouseUp, click] in this order and with default main-button(0) when clicked', async () => {
            ...
            const mouseUp = vi.fn();
            const click = vi.fn();
            const { container } = render(React.createElement("button", { onMouseDown: mouseDown, onMouseUp: mouseUp, onClick: click }, "bob"));
            const driver = jsdomReactUniDriver(container);
            await driver.$('button').click();
        ...
        it('should trigger [focus] on click', async () => {
            const focus = vi.fn();
            const { container } = render(React.createElement("button", { onFocus: focus }, "bob"), {
                attachToBody: false,
            });
            const driver = jsdomReactUniDriver(container);
            await driver.$('button').click();
        ...
        it('should trigger [mousedown, focus, mouseup, click] events on click', async () => {
            ...
            const mouseup = vi.fn();
            const click = vi.fn();
            const { container } = render(React.createElement("button", { onMouseDown: mousedown, onFocus: focus, onMouseUp: mouseup, onClick: click }, "bob"), { attachToBody: false });
            const driver = jsdomReactUniDriver(container);
            await driver.$('button').click();
            expect(mousedown).toHaveBeenCalledTimes(1);
            expect(focus).toHaveBeenCalledTimes(1);
        ...
        describe('on input[type=checkbox]', () => {
            it('should trigger [mousedown, focus, mouseup, click, input, change] events', async () => {
                ...
                const input = vi.fn();
                const change = vi.fn();
                const { container } = render(React.createElement("input", { type: "checkbox", onMouseDown: mousedown, onFocus: focus, onMouseUp: mouseup, onClick: click, onInput: input, onChange: change }), { attachToBody: false });
                const driver = jsdomReactUniDriver(container);
            ...
            it('should trigger change event with a target object containing correct `checked` property', async () => {
                const change = vi.fn();
                const { container } = render(React.createElement("input", { type: "checkbox", onChange: change }));
                const driver = jsdomReactUniDriver(container);
                await driver.$('input').click();
        ...
        it('should trigger [focusA, blurA, focusB] when clicking two buttons', async () => {
            ...
            const { container } = render(React.createElement("div", null,
                React.createElement("button", { id: "A", onFocus: focusA, onBlur: blurA }, "button A"),
                React.createElement("button", { id: "B", onFocus: focusB }, "button B")));
            const driver = jsdomReactUniDriver(container);
        ...
        it('should trigger [focusA, blurA] when clicking enabled and disabled button', async () => {
            ...
            const { container } = render(React.createElement("div", null,
                React.createElement("button", { id: "A", onFocus: focusA, onBlur: blurA }, "button A"),
                React.createElement("button", { id: "B", onFocus: focusB, disabled: true }, "button B")));
            const driver = jsdomReactUniDriver(container);
            await driver.$('button#A').click();
            await driver.$('button#B').click();
            expect(focusA).toHaveBeenCalledTimes(2); // TODO - should be 1
        ...
        it('should trigger blur on active element when clicking an svg', async () => {
            const blurA = vi.fn();
            const { container } = render(React.createElement("div", null,
                React.createElement("button", { id: "A", onBlur: blurA }, "button A"),
                React.createElement("svg", { id: "B", width: "100", height: "100" },
