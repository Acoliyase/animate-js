...
exports.cssScrollTo = cssScrollTo;
exports.fallbackScrollIntoView = fallbackScrollIntoView;
...
function fallbackScrollIntoView(target, container, duration) {
  if (!(container != null && container.current) || !(target != null && target.current)) return;
  let startX;
  let startY;
  ...
  const containerRect = isContainerWindow ? {
    top: 0,
    left: 0
  ...
  function animateScroll(currentTime) {
    ...
    if (isContainerWindow) {
      ...
      container == null || (_container$current = container.current) == null || _container$current.scrollTo({
        top: scrollToY,
        left: scrollToX
      });
    }
    if (progress < 1) {
      ...
      if (!isContainerWindow) {
        ...
        container == null || (_container$current2 = container.current) == null || _container$current2.style.removeProperty('scroll-snap-type');
      }
    }
  }
  requestAnimationFrame(animateScroll);
}
function cssScrollTo({
  scroller,
  fromX,
  toX,
  fromY,
  ...
  if (changeX === 0 && changeY === 0) {
    return {
      scrollingPromise: Promise.resolve({
        x: toX,
        y: toY
      })
    };
  ...
  if ((0, _navigatorUtils.isHTMLElement)(scroller)) {
  ...
  const scrollTransitionEvent = new CustomEvent('scrollTransition', {
    detail: {
      changeX: changeX / intervals,
      changeY: changeY / intervals
    }
  });
  ...
  const scrollingPromise = new _asyncUtils.Deferred();
