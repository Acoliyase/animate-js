...
if(style.objectPosition){style.objectPosition=imageStyle.objectPosition.replace(/(center|bottom)$/,'top');}return style;}function getSourceSetsTargetHeightByEffect(sourceSets,offsetWidth,offsetHeight,screenHeight,services){const sourceSetsTargetHeights={};sourceSets.forEach(_ref=>{let{mediaQuery,scrollEffect}=_ref;sourceSetsTargetHeights[mediaQuery]=services.getMediaDimensionsByEffect(scrollEffect,offsetWidth,offsetHeight,screenHeight).height;});return sourceSetsTargetHeights;}function computeSrcSets(measures,imageInfo,envConsts){const{sourceSets}=imageInfo;if(!sourceSets||!sourceSets.length){return;}const mediaToUri={};sourceSets.forEach(_ref2=>{let{mediaQuery,crop,focalPoint}=_ref2;const imageInfoClone={...imageInfo,targetHeight:(measures.sourceSetsTargetHeights||{})[mediaQuery]||0,imageData:{...imageInfo.imageData,crop,focalPoint}};const imageComputedProperties=getImageComputedProperties(imageInfoClone,envConsts,'img');mediaToUri[mediaQuery]=imageComputedProperties.uri||'';});return mediaToUri;}function measure(id,measures,domNodes,_ref3,services){let{containerElm,bgEffect='none',sourceSets}=_ref3;const innerImage=domNodes.image;const wixImage=domNodes[id];const screenHeight=getScreenHeight(services.getScreenHeightOverride==null?void 0:services.getScreenHeightOverride());// override positioning and scaling of image (SiteBackground mobile override behaviour)
