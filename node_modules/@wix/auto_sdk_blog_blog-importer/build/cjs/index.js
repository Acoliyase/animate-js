...
function stopImport(payload) {
  function __stopImport({ host }) {
    const metadata = {
      entityFqdn: "wix.blog.v1.wordpress_import",
      method: "POST",
      methodFqn: "com.wix.blog.WordpressImportService.StopImport",
      packageName: PACKAGE_NAME,
      migrationOptions: {
        optInTransformResponse: true
      },
      url: resolveComWixBlogWordpressImportServiceUrl({
        protoPath: "/v1/wordpress-import/stop",
        data: payload,
        host
      }),
      data: payload
...
async function stopImport2() {
  const { httpClient, sideEffects } = arguments[0];
  const payload = (0, import_rename_all_nested_keys.renameKeysFromSDKRequestToRESTRequest)({});
  const reqOpts = stopImport(payload);
  sideEffects?.onSiteCall?.();
  try {
    const result = await httpClient.request(reqOpts);
    sideEffects?.onSuccess?.(result);
...
function stopImport3(httpClient) {
  return () => stopImport2(
    // @ts-ignore
    { httpClient }
  );
...
var stopImport4 = /* @__PURE__ */ (0, import_rest_modules3.createRESTModule)(stopImport3);
