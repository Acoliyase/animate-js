...
var AbstractSentryBrowserClient = class {
  constructor(options) {
    ...
    this.validateOptions = (options) => {
      const missingSDKMethods = MandatorySDKMethods.filter(
        (method) => {
          var _a;
          return typeof ((_a = options.sentrySDK) == null ? void 0 : _a[method]) !== "function";
        }
      );
      if (missingSDKMethods.length > 0) {
        throw new Error(
          "Missing the following options.sentrySDK methods: ".concat(missingSDKMethods.join(", "))
        );
      }
    };
    this.validateOptions(options);
  ...
  init() {
    ...
    const sentryClient = new sentrySDK.BrowserClient({
      dsn,
      transport: transport != null ? transport : sentrySDK.makeFetchTransport,
  ...
  loadFullSDK(handler) {
    if (this.isFullSDK()) {
      handler == null ? void 0 : handler(this.init());
      return;
    }
    if (this.promise) {
    ...
    this.promise = new Promise((resolve) => {
      this.options.sentrySDK.onLoad(() => {
        ...
        handler == null ? void 0 : handler(resolvedState);
      });
    });
    ...
    this.captureException = (error, captureContext) => {
      this.loadFullSDK(({ sentryClient, scope }) => {
        ...
        sentryClient.captureException(
          ...
          {
            captureContext: {
              level: (_a = captureContext == null ? void 0 : captureContext.level) != null ? _a : "error",
              tags: captureContext == null ? void 0 : captureContext.tags,
              contexts: captureContext == null ? void 0 : captureContext.contexts
            }
          },
          scope
        );
    ...
    this.captureMessage = (message, captureContext) => {
      this.loadFullSDK(({ sentryClient, scope }) => {
        ...
        sentryClient.captureMessage(
          message,
          (_a = captureContext == null ? void 0 : captureContext.level) != null ? _a : "error",
          {
            captureContext: {
              level: (_b = captureContext == null ? void 0 : captureContext.level) != null ? _b : "error",
              tags: captureContext == null ? void 0 : captureContext.tags,
              contexts: captureContext == null ? void 0 : captureContext.contexts
            }
          },
      ...
      const simulateAndReportFailedSpan = async (error2, startTime2) => {
        ...
        this.loadFullSDK(async ({ sentrySDK, scope }) => {
          try {
            ...
            if ((e == null ? void 0 : e.message) !== (error2 == null ? void 0 : error2.message)) {
              throw e;
            }
          }
        });
...
// src/factory.ts
var unsupportedEnvironmentMessage = (version) => 'Sentry SDK version "'.concat(version, '" is not supported. Please make sure to use monitoring SDK only in supported environments.');
var major = (version) => {
  try {
...
var createSentryBrowserClient = ({
  ...
  try {
    ...
    return createFallbackMonitoringClient(
      "Failed to initialize monitoring client: ".concat(e.message)
    );
  }
};
export {
