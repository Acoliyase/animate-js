declare const isPromise: <T>(value: Promise<T> | T) => value is Promise<T>;
declare const runWithMaybePromise: <T>(value: Promise<T> | T, cb?: (value: T, isPromise: boolean) => void) => typeof value;
declare const runWithMaybeDelay: (cb?: Function, maybeTimeCompensation?: number) => void;
declare const generateRandomId: () => number;
...
interface ManualSpanRegistryOptions {
    forceTransaction: boolean;
}
interface ManualSpanRegistry {
    createSpan(spanOptions: StartSpanOptions, sentryOptionsOrPromise: SentryOptions | Promise<SentryOptions>): ManualSpan;
    getSpan(spanOptions: StartSpanOptions): ManualSpan | undefined;
}
declare class ManualSpanRegistryImpl implements ManualSpanRegistry {
    private options;
    private spanMap;
    constructor(options: ManualSpanRegistryOptions);
    createSpan(spanOptions: StartSpanOptions, sentryOptionsOrPromise: SentryOptions | Promise<SentryOptions>): ManualSpan;
    getSpan(spanOptions: Pick<StartSpanOptions, 'name'>): ManualSpan | undefined;
}
declare const createManualSpanRegistry: (options: ManualSpanRegistryOptions) => ManualSpanRegistryImpl;

export { type ManualSpanRegistry, type ManualSpanRegistryOptions, createManualSpanRegistry, generateRandomId, isPromise, runWithMaybeDelay, runWithMaybePromise };
