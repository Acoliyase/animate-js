...
async function typedQueryStoresLocations(query) {
  const { httpClient, sideEffects } = arguments[1];
  const payload = renameKeysFromSDKRequestToRESTRequest({ query });
  const reqOpts = queryStoresLocations(payload);
  sideEffects?.onSiteCall?.();
  try {
    const result = await httpClient.request(reqOpts);
    sideEffects?.onSuccess?.(result);
...
function typedQueryStoresLocations2(httpClient) {
  return (query) => typedQueryStoresLocations(
    query,
    // @ts-ignore
    { httpClient }
  );
...
function customQueryStoresLocations(httpClient) {
  const router = createQueryOverloadRouter({
    builderQueryFunction: () => queryStoresLocations3(httpClient)(),
    typedQueryFunction: (query) => typedQueryStoresLocations2(httpClient)(query),
...
var queryStoresLocations4 = /* @__PURE__ */ createRESTModule(customQueryStoresLocations);
export {
  LocationType,
  SortOrder,
  WebhookIdentityType,
