...
async function typedQueryDiscountRules(query) {
  const { httpClient, sideEffects } = arguments[1];
...
function typedQueryDiscountRules2(httpClient) {
  return (query) => typedQueryDiscountRules(
    query,
    // @ts-ignore
    { httpClient }
  );
...
function customQueryDiscountRules(httpClient) {
  const router = createQueryOverloadRouter({
    builderQueryFunction: () => queryDiscountRules3(httpClient)(),
    typedQueryFunction: (query) => typedQueryDiscountRules2(httpClient)(query),
