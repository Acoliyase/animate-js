// src/events-schedule-v1-schedule-item-schedule-bookmarks.universal.ts
...
// src/events-schedule-v1-schedule-item-schedule-bookmarks.http.ts
...
var PACKAGE_NAME = "@wix/auto_sdk_events_schedule-bookmarks";
...
async function listBookmarks2(eventId) {
  const { httpClient, sideEffects } = arguments[1];
  const payload = renameKeysFromSDKRequestToRESTRequest({ eventId });
  const reqOpts = listBookmarks(payload);
...
async function createBookmark2(itemId, eventId) {
  const { httpClient, sideEffects } = arguments[2];
  const payload = renameKeysFromSDKRequestToRESTRequest({
    itemId,
    eventId
  });
  const reqOpts = createBookmark(payload);
  sideEffects?.onSiteCall?.();
  try {
    const result = await httpClient.request(reqOpts);
    sideEffects?.onSuccess?.(result);
...
async function deleteBookmark2(itemId, eventId) {
  const { httpClient, sideEffects } = arguments[2];
  const payload = renameKeysFromSDKRequestToRESTRequest({
    itemId,
    eventId
  });
  const reqOpts = deleteBookmark(payload);
  sideEffects?.onSiteCall?.();
  try {
    const result = await httpClient.request(reqOpts);
    sideEffects?.onSuccess?.(result);
...
// src/events-schedule-v1-schedule-item-schedule-bookmarks.public.ts
function listBookmarks3(httpClient) {
  return (eventId) => listBookmarks2(
    eventId,
    // @ts-ignore
    { httpClient }
  );
}
function createBookmark3(httpClient) {
  return (itemId, eventId) => createBookmark2(
    itemId,
    eventId,
    // @ts-ignore
    { httpClient }
...
function deleteBookmark3(httpClient) {
  return (itemId, eventId) => deleteBookmark2(
    itemId,
    eventId,
    // @ts-ignore
    { httpClient }
...
// src/events-schedule-v1-schedule-item-schedule-bookmarks.context.ts
import { createRESTModule } from "@wix/sdk-runtime/rest-modules";
var listBookmarks4 = /* @__PURE__ */ createRESTModule(listBookmarks3);
var createBookmark4 = /* @__PURE__ */ createRESTModule(createBookmark3);
var deleteBookmark4 = /* @__PURE__ */ createRESTModule(deleteBookmark3);
export {
  ScheduleStatus,
  SortOrder,
  StateFilter,
  createBookmark4 as createBookmark,
  deleteBookmark4 as deleteBookmark,
  listBookmarks4 as listBookmarks
};
