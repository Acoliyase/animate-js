...
import { isMultipleElementsWithSelectorError, isNoElementWithSelectorError } from '../../../UniDriverBase/errors';
import { delay } from '../../../utils/delay';
export const runSelectorTestSuite = (params) => {
    const { isStorybook, setup } = params;
    describe('$', () => {
        describe('text()', () => {
            it('returns text of a element', async ({ mount }) => {
                await runTest(setup, { mount }, isStorybook ? { story: 'with-item' } : { items: [itemCreator({ label: 'Bob' })] }, async ({ driver }) => {
        ...
        describe('hasClass()', () => {
            ...
            it('returns true when there are multiple class names', async ({ mount }) => {
                await runTest(setup, { mount }, isStorybook
                    ? { story: 'with-two-items' }
                    : {
                        items: [itemCreator({ label: 'Bob' }), itemCreator({ label: 'David' })],
                    }, async ({ driver }) => {
        ...
        describe('value()', () => {
            it('returns value of given input', async ({ mount }) => {
                await runTest(setup, { mount }, isStorybook ? { story: 'with-initial-text' } : { initialText: 'hello', items: [] }, async ({ driver }) => {
        ...
        describe('enterValue()/enterText()', () => {
            const runInputTests = (fnName) => {
                ...
                it('should do nothing when input is disabled', async ({ mount }) => {
                    await runTest(setup, { mount }, isStorybook ? { story: 'disabled-input' } : { initialText: 'other text', inputDisabled: true, items: [] }, async ({ driver }) => {
                        await driver.$('header input')[fnName]('hey there');
                        expect(await driver.$('header input').value()).toBe('other text');
                    });
                });
                it('should do nothing when input is readOnly', async ({ mount }) => {
                    await runTest(setup, { mount }, isStorybook ? { story: 'readonly-input' } : { initialText: 'other text', inputReadOnly: true, items: [] }, async ({ driver }) => {
                        await driver.$('header input')[fnName]('hey there');
                        expect(await driver.$('header input').value()).toBe('other text');
                    });
                });
                it('shout wait between key presses when delay option is passed', async ({ mount }) => {
                    await runTest(setup, { mount }, isStorybook ? { story: 'base-empty-initial-text' } : { initialText: '', items: [] }, async ({ driver }) => {
                        const sleepPromise = delay(500).then(async () => {
            ...
            it('rejects the promise with an error when more than 1 element exists', async ({ mount }) => {
                ...
                await runTest(setup, { mount }, isStorybook ? { story: 'with-two-items' } : { items }, async ({ driver }) => {
                    const error = await driver
                        ...
                        .exists()
                        .catch((e) => e);
                    expect(isMultipleElementsWithSelectorError(error)).toBe(true);
                });
            });
        });
        describe('getNative()', () => {
        ...
        describe('attr()', () => {
            it('returns null if attr does not exist', async ({ mount }) => {
                const items = [itemCreator({ label: 'Bob' })];
                await runTest(setup, { mount }, isStorybook ? { story: 'with-item' } : { items }, async ({ driver }) => {
                    expect(await driver.$('[data-hook="todo-item"]').attr('data-value')).toBe(null);
                });
            });
            it('returns attribute value', async ({ mount }) => {
                const itemsToShow = [
                    itemCreator({ completed: false, id: 'bob', label: 'Bob' }),
                    itemCreator({ completed: false, id: 'alice', label: 'Alice' }),
                ];
                await runTest(setup, { mount }, isStorybook ? { story: 'items-with-ids' } : { items: itemsToShow }, async ({ driver }) => {
            ...
            it('returns empty string', async ({ mount }) => {
                const items = [itemCreator({ completed: false, id: '', label: 'Bob' })];
                await runTest(setup, { mount }, isStorybook ? { story: 'item-with-empty-id' } : { items }, async ({ driver }) => {
        ...
        describe('prop()', () => {
            ...
            it('returns the correct prop for items in list', async ({ mount }) => {
                const items = [itemCreator({ completed: false, label: 'Bob' }), itemCreator({ completed: true, label: 'David' })];
                await runTest(setup, { mount }, isStorybook ? { story: 'with-two-items' } : { items }, async ({ driver }) => {
        ...
        it('rejects with the right error on action when an element does not exist given selector', async ({ mount }) => {
            await runTest(setup, { mount }, isStorybook ? { story: 'base' } : { items: [] }, async ({ driver }) => {
                const err = await driver
                    ...
                    .text()
                    .catch((e) => e);
                expect(isNoElementWithSelectorError(err)).toBe(true);
            });
        });
        it('rejects with the right error on action when an more than 1 element exist given selector', async ({ mount }) => {
            ...
            await runTest(setup, { mount }, isStorybook ? { story: 'with-two-items' } : { items }, async ({ driver }) => {
                const err = await driver
                    ...
                    .text()
                    .catch((e) => e);
                expect(isMultipleElementsWithSelectorError(err)).toBe(true);
            });
        });
    });
};
