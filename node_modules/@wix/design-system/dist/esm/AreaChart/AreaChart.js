...
import { st, classes, stVars } from './AreaChart.st.css.js';
...
class AreaChart extends React.PureComponent {
    constructor() {
        ...
        this.handleHoverThrottled = throttle(this.handleHover, 100);
    ...
    render() {
        const { data = [], tooltipContent, maxYTicksLimit, dataHook, className, } = this.props;
        const labels = data.map(i => i.label);
        const dataset = data.map(i => i.value);
        return (React.createElement("section", { className: st(classes.lineChart, className), "data-hook": dataHook },
            React.createElement(Line, { redraw: true, data: {
                    labels,
                    datasets: [
                        {
                            ...DATASET_PROPS,
                        ...
                        callbacks: {
                            ...
                            label: tooltipItem => {
                                return (tooltipContent &&
                                    tooltipContent([...data][tooltipItem.index], tooltipItem.index));
                            },
                        },
                    },
                    onHover: this.handleHoverThrottled,
                } }),
            React.createElement("div", { ref: line => (this.cursorLine = line), className: classes.lineChartCursor })));
    }
}
