...
import { dataHooks } from './AnalyticsSummaryCard.constants';
export const analyticsSummaryCardDriverFactory = (base, body) => {
    const getLoader = () => loaderUniDriverFactory(findByHook(base, dataHooks.analyticsSummaryCardLoader), body);
    const getValue = () => textUniDriverFactory(findByHook(base, dataHooks.analyticsSummaryCardValue));
    const getTitle = () => textUniDriverFactory(findByHook(base, dataHooks.analyticsSummaryCardTitle));
    const getValueTooltip = () => tooltipDriverFactory(base.$(`[data-hook="${dataHooks.analyticsSummaryCardValueTooltip}"]`), body);
    const getTitleTooltip = () => tooltipDriverFactory(base.$(`[data-hook="${dataHooks.analyticsSummaryCardTitleTooltip}"]`), body);
    const getTrendIndicator = () => trendIndicatorDriverFactory(findByHook(base, dataHooks.analyticsSummaryCardTrend));
...
//# sourceMappingURL=AnalyticsSummaryCard.uni.driver.js.map