...
const strip_js_1 = require("./strip.js");
const column_js_1 = require("./column.js");
const layoutsStore_js_1 = tslib_1.__importDefault(require("./layoutsStore.js"));
...
class Layouter {
    ...
    updateParams(layoutParams) {
        ...
        this.styleParams = utils_js_1.utils.addDefaultStyleParams(layoutParams.styleParams);
        this.container = utils_js_1.utils.convertContainer(layoutParams.container, this.styleParams);
    ...
    calcNumberOfColumns(galleryWidth, targetItemSize) {
        let numOfCols = 1;
        if (this.styleParams[pro_gallery_lib_1.optionsMap.layoutParams.structure.layoutOrientation] === 'VERTICAL') {
    ...
    findShortestColumn(columns, groupIdx) {
        ...
        if (this.styleParams[pro_gallery_lib_1.optionsMap.layoutParams.structure.groupsOrder] === 'LEFT_TO_RIGHT') {
    ...
    prepareLayoutParams() {
        if (this.useExistingLayout && this.pointer > 0) {
                    ...
                    if (this.scrollDirection === 'HORIZONTAL') {
                        ...
                        if (lastStrips) {
                            ...
                            this.strip = new strip_js_1.Strip({
                                ...
                                groupsPerStrip: this.styleParams[pro_gallery_lib_1.optionsMap.layoutParams.groups.numberOfGroupsPerRow],
                                scrollDirection: this.styleParams[pro_gallery_lib_1.optionsMap.layoutParams.structure.scrollDirection],
                                targetItemSize: this.targetItemSize,
                            });
                        }
                    }
            ...
            if (this.styleParams.forceFullHeight) { //v5 TODO check and remove this API
                ...
                this.targetItemSize =
                    Math.floor(gallerySizeVal) +
                        Math.ceil(2 *
                            (this.styleParams[pro_gallery_lib_1.optionsMap.layoutParams.structure.itemSpacing] / 2 -
                                this.styleParams[pro_gallery_lib_1.optionsMap.layoutParams.structure.gallerySpacing]));
            }
            ...
            this.targetItemSize = this.styleParams[pro_gallery_lib_1.optionsMap.layoutParams.structure.layoutOrientation] === 'VERTICAL'
                ? Math.floor(this.galleryWidth / this.numOfCols)
                : this.targetItemSize;
            const { externalInfoWidth } = this.styleParams;
            const columnWidths = this.styleParams[pro_gallery_lib_1.optionsMap.layoutParams.structure.columnRatios];
            const imageMargin = this.styleParams[pro_gallery_lib_1.optionsMap.layoutParams.structure.itemSpacing];
            const cropRatio = this.styleParams[pro_gallery_lib_1.optionsMap.layoutParams.crop.ratios];
            ...
            if (columnWidths && columnWidths.length > 0) {
                ...
                const columnMultiplier = this.galleryWidth / columnWidthsArr.reduce((a, b) => a + b, 0);
                columnWidthsArr = columnWidthsArr.map((col) => Math.round((col *= columnMultiplier)));
            }
            let totalLeft = 0;
            ...
            this.columns = Array(this.numOfCols)
                ...
                return new column_js_1.Column(idx, colWidth, curLeft, fixedCubeHeight, infoWidth);
            });
            this.maxLoops = this.srcItems.length * 10;
        }
    }
    createLayout(layoutParams) {
        ...
        while (this.srcItems[this.pointer]) {
            ...
            if (this.styleParams[pro_gallery_lib_1.optionsMap.layoutParams.structure.layoutOrientation] === 'HORIZONTAL') {
                //---------------------| STRIPS GALLERY |----------------------//
                ...
                if (this.isLastImage && this.strip.hasGroups) {
                    if (this.styleParams[pro_gallery_lib_1.optionsMap.layoutParams.structure.scrollDirection] === 'HORIZONTAL') {
                        this.strip.height =
        ...
        this.height =
            this.galleryHeight -
                (this.styleParams[pro_gallery_lib_1.optionsMap.layoutParams.structure.itemSpacing] / 2 - this.styleParams[pro_gallery_lib_1.optionsMap.layoutParams.structure.gallerySpacing]) * 2;
    ...
    getMaxGroupSize() {
        let _maxGroupSize = 1;
        try {
            const groupTypes = typeof this.styleParams[pro_gallery_lib_1.optionsMap.layoutParams.groups.allowedGroupTypes] === 'string' &&
                this.styleParams[pro_gallery_lib_1.optionsMap.layoutParams.groups.allowedGroupTypes].length > 0
                ? this.styleParams[pro_gallery_lib_1.optionsMap.layoutParams.groups.allowedGroupTypes].split(',')
                : this.styleParams[pro_gallery_lib_1.optionsMap.layoutParams.groups.allowedGroupTypes];
            _maxGroupSize =
                groupTypes.length > 0
                    ? groupTypes.reduce((curSize, groupType) => Math.max(curSize, parseInt(groupType)), 1)
                    : Number(groupTypes);
            _maxGroupSize = Math.min(_maxGroupSize, this.styleParams[pro_gallery_lib_1.optionsMap.layoutParams.groups.groupSize]);
        }
        catch (e) {
