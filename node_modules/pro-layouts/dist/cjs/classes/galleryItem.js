...
const item_js_1 = require("../logic/item.js");
class GalleryItem {
    constructor(config) {
    ...
    mapItemConfig(config) {
        ...
        return Object.assign(Object.assign({}, config), { dto, scheme: Object.assign(Object.assign({}, config.scheme), { maxHeight: dto.height, maxWidth: dto.width }) });
    }
    update(config) {
        ...
        if (config.scheme) {
            ...
            this.processScheme(new item_js_1.Item({ dto }).scheme);
        }
        if (this.dto) {
    ...
    renderProps(config) {
        const style = Object.assign({ ratio: this.ratio, bgColor: this.bgColor, maxWidth: this.maxWidth, maxHeight: this.maxHeight, infoWidth: this.infoWidth, infoHeight: this.infoHeight, orientation: this.orientation }, this.style);
        const itemProps = Object.assign({ className: 'image', key: this.key, idx: this.idx, photoId: this.photoId, id: this.id, hash: this.id, html: this.html, type: this.type, isVideoPlaceholder: this.isVideoPlaceholder, videoPlaceholderUrl: this.videoPlaceholderUrl, url: this.url, alt: this.alt, htmlContent: this.htmlContent, directLink: this.directLink, directShareLink: this.directShareLink, linkUrl: this.linkUrl, linkType: this.linkType, linkOpenType: this.linkOpenType, linkData: this.linkData, title: this.title, fileName: this.fileName, description: this.description, createUrl: this.createUrl, createMagnifiedUrl: this.createMagnifiedUrl, cubeImages: this.cubeImages, cubeType: this.cubeType, cropRatio: this.cropRatio, isTransparent: this.isTransparent, offset: this.offset, style, isDemo: this.isDemo, videoUrl: this.videoUrl, isExternalVideo: this.isExternalVideo, hasSecondaryMedia: this.hasSecondaryMedia, scene: this.scene }, config);
        if (this.hasSecondaryMedia) {
            itemProps.secondaryMediaItem = Object.assign(Object.assign({}, this.secondaryMediaItem.renderProps()), { style: Object.assign(Object.assign({}, style), { bgColor: this.secondaryMediaItem.bgColor }) });
        }
        return itemProps;
    }
    getDataForShop() {
    ...
    processedMediaUrl(resizeMethod, requiredWidth, requiredHeight, sharpParams, createMultipleUrls = false) {
        var _a;
        const mediaUrl = (item, url, resizeMethod, requiredWidth, requiredHeight, sharpParams = null, focalPoint = null) => {
            let mediaUrl;
            if (typeof this.createMediaUrl === 'function') {
                try {
                    const str = String(pro_gallery_lib_1.utils.hashCode(JSON.stringify({
                        url,
                        resizeMethod,
                        requiredWidth,
                        requiredHeight,
        ...
        const thumbSize = 250;
        const focalPoint = resizeMethod === pro_gallery_lib_1.GALLERY_CONSTS.resizeMethods.FILL && this.isCropped && this.focalPoint;
        const urls = {};
        let imgUrl = this.url;
        urls[pro_gallery_lib_1.GALLERY_CONSTS.urlTypes.THREE_D] = () => this.url;
        if (this.is3D) {
            imgUrl = this.poster.url;
        }
        if (this.isText) {
            return Object.assign({}, ...Object.values(pro_gallery_lib_1.GALLERY_CONSTS.urlTypes).map((value) => ({
                [value]: () => '',
            })));
        }
            ...
            if (pro_gallery_lib_1.utils.isExternalUrl(this.url)) {
                urls[pro_gallery_lib_1.GALLERY_CONSTS.urlTypes.VIDEO] = () => this.url;
            }
            else {
                urls[pro_gallery_lib_1.GALLERY_CONSTS.urlTypes.VIDEO] = () => mediaUrl(this, this.url, pro_gallery_lib_1.GALLERY_CONSTS.resizeMethods.VIDEO, requiredWidth, requiredHeight);
            }
        }
        urls[pro_gallery_lib_1.GALLERY_CONSTS.urlTypes.HIGH_RES] = () => mediaUrl(this, imgUrl, resizeMethod, requiredWidth, requiredHeight, sharpParams, focalPoint);
        urls[pro_gallery_lib_1.GALLERY_CONSTS.urlTypes.LOW_RES] = () => {
            const resizedRatio = this.resizeWidth / this.resizeHeight;
            return mediaUrl(this, imgUrl, this.cubeImages && resizeMethod !== pro_gallery_lib_1.GALLERY_CONSTS.resizeMethods.FIT
                ? pro_gallery_lib_1.GALLERY_CONSTS.resizeMethods.FILL
                : pro_gallery_lib_1.GALLERY_CONSTS.resizeMethods.FIT, thumbSize * resizedRatio, thumbSize, Object.assign(Object.assign({}, sharpParams), { quality: 30, blur: 30 }), focalPoint);
        };
        return urls;
    }
    resetUrls() {
    ...
    get cubeTypeResizeMethod() {
        return {
            [pro_gallery_lib_1.GALLERY_CONSTS.layoutParams_crop_method.FIT]: pro_gallery_lib_1.GALLERY_CONSTS.resizeMethods.FIT,
            [pro_gallery_lib_1.GALLERY_CONSTS.layoutParams_crop_method.FILL]: pro_gallery_lib_1.GALLERY_CONSTS.resizeMethods.FILL,
        }[this.cubeType];
    }
    createMagnifiedUrl(scale = 1) {
        if (!this.urls.magnified_url) {
            const { innerWidth, innerHeight } = this.style;
            this.urls.magnified_url = this.processedMediaUrl(this.cubeTypeResizeMethod, innerWidth * scale, innerHeight * scale, this.sharpParams, true);
        }
        return this.urls.magnified_url[pro_gallery_lib_1.GALLERY_CONSTS.urlTypes.HIGH_RES]();
    }
    get resized_url() {
        if (!this.urls.resized_url) {
            this.urls.resized_url = this.processedMediaUrl(this.cubeTypeResizeMethod, this.resizeWidth, this.resizeHeight, this.sharpParams);
        }
        return this.urls.resized_url;
    }
    get multi_url() {
        if (!this.urls.multi_url) {
            this.urls.multi_url = this.processedMediaUrl(this.cubeTypeResizeMethod, this.resizeWidth, this.resizeHeight, this.sharpParams, true);
        }
    ...
    get isCropped() {
        return this.cubeImages && this.cubeTypeResizeMethod === pro_gallery_lib_1.GALLERY_CONSTS.resizeMethods.FILL;
    }
    get focalPoint() {
    ...
    get alt() {
        return (pro_gallery_lib_1.utils.isMeaningfulString(this.metadata.alt) && this.metadata.alt) || '';
    }
    set alt(value) {
    ...
    get linkOpenType() {
        ...
        else if (this.metadata.link && !pro_gallery_lib_1.utils.isUndefined(this.metadata.link.targetBlank)) {
