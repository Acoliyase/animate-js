...
exports.use3DItem = void 0;
const pro_gallery_lib_1 = require("pro-gallery-lib");
const _3dManager_js_1 = require("../../helpers/3dManager.js");
...
function use3DItem(props) {
    const canvasRef = (0, react_1.useRef)(null);
    const { sceneManager, render } = (0, _3dManager_js_1.useSceneManager)();
    const [isLoaded, setIsLoaded] = (0, react_1.useState)(false);
    const sceneParams = mapSceneToStyleParams(props.scene || {}, props.options);
    const loadedManager = (0, react_1.useMemo)(() => {
        if (!props.shouldPlay) {
            return sceneManager;
        }
        if (sceneManager) {
        ...
        manager.opacity = 0;
        manager.model
            .load3DModel(props.createUrl(pro_gallery_lib_1.GALLERY_CONSTS.urlSizes.RESIZED, pro_gallery_lib_1.GALLERY_CONSTS.urlTypes.THREE_D))
            .then(({ punctualLights }) => {
    ...
    const postLoadUpdate = (0, react_1.useCallback)(({ behaviourParams_item_threeDimensionalScene_transform_rotation, behaviourParams_item_threeDimensionalScene_transform_scale, behaviourParams_item_threeDimensionalScene_transform_position, behaviourParams_item_threeDimensionalScene_controls_enablePan, behaviourParams_item_threeDimensionalScene_controls_enableRotate, behaviourParams_item_threeDimensionalScene_controls_enableZoom, behaviourParams_item_threeDimensionalScene_controls_enableAutoRotate, }) => {
        if (!loadedManager) {
            return;
        }
        const rotation = pro_gallery_lib_1.GALLERY_CONSTS.parse3DDimensions(behaviourParams_item_threeDimensionalScene_transform_rotation);
        const scale = pro_gallery_lib_1.GALLERY_CONSTS.parse3DDimensions(behaviourParams_item_threeDimensionalScene_transform_scale);
        const position = pro_gallery_lib_1.GALLERY_CONSTS.parse3DDimensions(behaviourParams_item_threeDimensionalScene_transform_position);
