...
exports.useSceneManager = exports.createSceneManager = void 0;
...
const RoomEnvironment_js_1 = require("three/examples/jsm/environments/RoomEnvironment.js");
const three_1 = require("three");
const GLOBAL_LOADING_MANAGER = new three_1.LoadingManager();
const DEFAULT_OPTIONS = {
    SUN_LIGHT_COLOR: 0xffffff,
    SUN_LIGHT_INTENSITY: 0.2,
    SUN_POSITION: [0.5, 0, 0.866],
...
function createSceneManager(container, canvas) {
    const scene = new THREE.Scene();
    const dracoLoader = new DRACOLoader_js_1.DRACOLoader(GLOBAL_LOADING_MANAGER);
    const gltfLoader = new GLTFLoader_js_1.GLTFLoader(GLOBAL_LOADING_MANAGER);
    gltfLoader.setDRACOLoader(dracoLoader);
    const width = container.clientWidth;
    const height = container.clientHeight;
    const camera = new THREE.PerspectiveCamera(DEFAULT_OPTIONS.CAMERA_FOV, width / height, DEFAULT_OPTIONS.CAMERA_NEAR, DEFAULT_OPTIONS.CAMERA_FAR);
    scene.add(camera);
    const controls = new OrbitControls_js_1.OrbitControls(camera, container);
    ...
    const initRenderer = () => {
        ...
        const envTexture = new THREE.PMREMGenerator(renderer).fromScene(new RoomEnvironment_js_1.RoomEnvironment()).texture;
    ...
    const api = {
        ...
        environment: {
            ...
            addByDirectionalLights(intensity = DEFAULT_OPTIONS.DIRECTIONAL_LIGHT_INTENSITY, color = DEFAULT_OPTIONS.DIRECTIONAL_LIGHT_COLOR) {
            ...
            addAmbientLight(intensity = DEFAULT_OPTIONS.AMBIENT_LIGHT_INTENSITY, color = DEFAULT_OPTIONS.AMBIENT_LIGHT_COLOR) {
            ...
            async loadHDR(url) {
                const { RGBELoader } = await Promise.resolve().then(() => tslib_1.__importStar(require(
        ...
        model: {
            ...
            addGround(opacity = DEFAULT_OPTIONS.GROUND_OPACITY) {
                const ground = new THREE.Mesh(new THREE.PlaneBufferGeometry(100, 100), new THREE.ShadowMaterial({ opacity: opacity }));
            ...
            transform: {
                setRotation(x, y, z) {
                    ...
                    // deg to rad
                    const radX = ((x !== null && x !== void 0 ? x : 0) * Math.PI) / 180;
                    const radY = ((y !== null && y !== void 0 ? y : 0) * Math.PI) / 180;
                    const radZ = ((z !== null && z !== void 0 ? z : 0) * Math.PI) / 180;
                ...
                setPosition(x, y, z) {
                    ...
                    manualPosition = true;
                    camera.position.x = x !== null && x !== void 0 ? x : 0;
                    camera.position.y = y !== null && y !== void 0 ? y : 0;
                    camera.position.z = z !== null && z !== void 0 ? z : 0;
                ...
                setScale(x, y, z) {
                    ...
                    model.scale.set(x !== null && x !== void 0 ? x : 1, y !== null && y !== void 0 ? y : 1, z !== null && z !== void 0 ? z : 1);
                },
            },
        },
        camera: {
...
exports.createSceneManager = createSceneManager;
function useSceneManager() {
    const [sceneManager, setSceneManager] = (0, react_1.useState)(null);
    const render = (0, react_1.useCallback)((container, canvas) => {
        if (!sceneManager) {
...
exports.useSceneManager = useSceneManager;
/**
 * from - https://github.com/donmccurdy/three-gltf-viewer/blob/main/src/viewer.js - MIT
 */
function addModel(scene, object, animations, camera, controls, manualPosition) {
...
function playAllAnimations(mixer, animations) {
...
//# sourceMappingURL=3dManager.js.map