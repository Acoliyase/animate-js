...
class CssScrollHelper {
    ...
    calcScrollCss({ id, items, options, container }) {
        ...
        const scrollAnimation = options[pro_gallery_lib_1.optionsMap.behaviourParams.gallery.scrollAnimation];
        if (scrollAnimation === pro_gallery_lib_1.GALLERY_CONSTS[pro_gallery_lib_1.optionsMap.behaviourParams.gallery.scrollAnimation].NO_EFFECT) {
            return [];
        }
        this.screenSize =
        ...
        if (options[pro_gallery_lib_1.optionsMap.layoutParams.structure.scrollDirection] ===
        ...
        this.maxHeight =
            (Math.ceil(((options[pro_gallery_lib_1.optionsMap.layoutParams.structure.scrollDirection] ===
                pro_gallery_lib_1.GALLERY_CONSTS[pro_gallery_lib_1.optionsMap.layoutParams.structure.scrollDirection].HORIZONTAL
                ? right
                : top) +
                this.screenSize) /
    ...
    createScrollSelectorsFunction({ id, item, options }) {
        const imageTop = options[pro_gallery_lib_1.optionsMap.layoutParams.structure.scrollDirection] ===
            pro_gallery_lib_1.GALLERY_CONSTS[pro_gallery_lib_1.optionsMap.layoutParams.structure.scrollDirection].HORIZONTAL
            ? item.offset.left - this.screenSize
            : item.offset.top - this.screenSize;
        const imageBottom = options[pro_gallery_lib_1.optionsMap.layoutParams.structure.scrollDirection] ===
            pro_gallery_lib_1.GALLERY_CONSTS[pro_gallery_lib_1.optionsMap.layoutParams.structure.scrollDirection].HORIZONTAL
        ...
        return (padding, suffix) => {
            ...
            while (from < to) {
                ...
                scrollClasses.push(`.${this.buildScrollClassName(id, largestDividerIdx, from)} ~ div #${sellectorDomId} ${suffix}`);
    ...
    createScrollAnimationsIfNeeded({ idx, options, createScrollSelectors }) {
        const scrollAnimation = options[pro_gallery_lib_1.optionsMap.behaviourParams.gallery.scrollAnimation];
        const { NO_EFFECT, FADE_IN, GRAYSCALE, SLIDE_UP, EXPAND, SHRINK, ZOOM_OUT, ONE_COLOR, MAIN_COLOR, BLUR } = pro_gallery_lib_1.GALLERY_CONSTS[pro_gallery_lib_1.optionsMap.behaviourParams.gallery.scrollAnimation];
        ...
        if (scrollAnimation === SLIDE_UP) {
            const axis = scrollDirection === pro_gallery_lib_1.GALLERY_CONSTS[pro_gallery_lib_1.optionsMap.layoutParams.structure.scrollDirection].HORIZONTAL ? 'X' : 'Y';
        ...
        if (scrollAnimation === ONE_COLOR) {
            const oneColorAnimationColor = options.oneColorAnimationColor && options.oneColorAnimationColor.value
