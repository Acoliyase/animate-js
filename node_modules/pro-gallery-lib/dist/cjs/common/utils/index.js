...
const lodash = tslib_1.__importStar(require("./lodash.js"));
const windowWrapper_js_1 = tslib_1.__importDefault(require("../window/windowWrapper.js"));
const viewModeWrapper_js_1 = require("../window/viewModeWrapper.js");
...
class Utils {
    ...
    shouldUseCache() {
        return !(0, viewModeWrapper_js_1.isEditMode)() && !(0, viewModeWrapper_js_1.isPreviewMode)() && !this.isSSR();
    }
    ...
    isMobileByProps() {
        const _isMobileByProps = () => {
            const deviceType = this.parseGetParam('deviceType') || windowWrapper_js_1.default.deviceType;
            const isMobileViewer = this.parseGetParam('showMobileView') === 'true';
            const deviceTypeMobile = (0, viewModeWrapper_js_1.isDeviceTypeMobile)();
            if (isMobileViewer) {
                return true;
            }
            else if (deviceType) {
    ...
    isUserAgentMobile() {
        try {
            const _isUserAgentMobile = () => {
                ...
                })(navigator.userAgent || navigator.vendor || windowWrapper_js_1.default.opera);
                return check;
            };
    ...
    isTouch() {
        const _isTouch = () => {
            var _a;
            const isTouchByProps = (0, viewModeWrapper_js_1.isDeviceTypeTouch)();
            const isTouchBrowser = ((_a = windowWrapper_js_1.default.navigator) === null || _a === void 0 ? void 0 : _a.maxTouchPoints) > 0 || // checking if navigator exists because of SSR
                'ontouchstart' in windowWrapper_js_1.default.document.documentElement;
    ...
    isTest() {
        try {
            return windowWrapper_js_1.default.isTest;
        }
        catch (e) {
            return false;
        }
    ...
    isLocal() {
        try {
            const host = windowWrapper_js_1.default.location.hostname;
    ...
    isDev() {
        return this.getOrPutFromCache('isDev', () => {
            return (this.shouldDebug('ph_local') ||
    ...
    isStoreGallery() {
        return this.getOrPutFromCache('isStoreGallery', () => {
            try {
                return windowWrapper_js_1.default.location.search.toLowerCase().indexOf('isstore') > -1;
            }
            catch (e) {
                if (this.isDev()) {
    ...
    isSSR() {
        return typeof global !== 'undefined' && typeof (global === null || global === void 0 ? void 0 : global.window) === 'undefined';
    }
    isOOI() {
        return (this.isSSR() ||
    ...
    isiOS() {
        return this.getOrPutFromCache('isiOS', () => {
            try {
                return /iPad|iPhone|iPod/.test(navigator.userAgent) && !windowWrapper_js_1.default.MSStream;
            }
            catch (e) {
                return false;
            }
    ...
    isiPhone() {
        return this.getOrPutFromCache('isiPhone', () => {
            try {
                return /iPhone/.test(navigator.userAgent) && !windowWrapper_js_1.default.MSStream;
            }
            catch (e) {
                return false;
            }
    ...
    isLandscape() {
        return this.getOrPutFromCache('isLandscape', () => {
            ...
            try {
                if (!this.isUndefined(windowWrapper_js_1.default.orientation)) {
                    return windowWrapper_js_1.default.orientation === 90 || windowWrapper_js_1.default.orientation === -90;
                }
                else {
                    const mql = windowWrapper_js_1.default.matchMedia('(orientation: landscape)');
    ...
    safeLocalStorage() {
        try {
            return localStorage ? localStorage : windowWrapper_js_1.default; // TrackJS errors, function returning null
        }
        catch (e) {
            return windowWrapper_js_1.default;
        }
    }
    shouldDebug(str) {
        try {
            return (!!this.safeLocalStorage()[str] ||
                (windowWrapper_js_1.default.debugApp || '').indexOf(str) >= 0 ||
    ...
    setStateAndLog(that, caller, state, callback) {
        if (this.isVerbose()) {
            ...
            const oldState = Object.assign({}, that.state);
            that.setState(state, () => {
    ...
    getScreenWidth() {
        if ((0, viewModeWrapper_js_1.isPreviewMode)() && this.isMobile()) {
        ...
        try {
            if (this.isLandscape()) {
                return Math.max(windowWrapper_js_1.default.screen.width, windowWrapper_js_1.default.screen.height);
            }
            else {
                return windowWrapper_js_1.default.screen.width;
            }
        }
        catch (e) {
            return 1920;
    ...
    getScreenHeight() {
        ...
        try {
            if (this.isLandscape()) {
                return Math.min(windowWrapper_js_1.default.screen.width, windowWrapper_js_1.default.screen.height);
            }
            else {
                return windowWrapper_js_1.default.screen.height;
            }
        }
        catch (e) {
            return 1200;
    ...
    getWindowWidth() {
        try {
            return windowWrapper_js_1.default.innerWidth || 980;
        }
        catch (e) {
            return 980;
        }
        ...
        try {
            if (this.isVerbose()) {
                const optionsStr = Object.entries(options)
                    .filter(([key, val]) => typeof val !== 'object' && String(key).indexOf('Expand') === -1 && String(key).indexOf('Color') === -1)
    ...
    isSingleItemHorizontalDisplay(options) {
        return (options.scrollDirection === index_js_1.default[optionsMap_js_1.default.layoutParams.structure.scrollDirection].HORIZONTAL &&
            options[optionsMap_js_1.default.layoutParams.groups.groupSize] === 1 &&
            options[optionsMap_js_1.default.layoutParams.crop.enable] &&
            options[optionsMap_js_1.default.layoutParams.crop.ratios].length === 1 &&
            options[optionsMap_js_1.default.layoutParams.crop.ratios][0] === '100%/100%');
    }
    ...
    isHeightSetByGallery(options) {
        return (options[optionsMap_js_1.default.layoutParams.structure.scrollDirection] ===
            index_js_1.default[optionsMap_js_1.default.layoutParams.structure.scrollDirection].VERTICAL &&
            !options[optionsMap_js_1.default.behaviourParams.gallery.vertical.loadMore.enable]); //v5 TODO!!! NEW STYLEPARAMS METHOD POSSIBLE BUG FOUND Could be that I need to add the horizontal gallery ratio thing here....
    }
}
